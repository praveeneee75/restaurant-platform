...
function thermalPrintHtml(job) {
  ...
  const rows = groupPrintableItems(payload.items).map((item, index) => `<tr><td>${index + 1}</td><td>${printEsc(item.name)}</td><td>${printEsc(item.quantity)}</td><td>${Number(item.price || 0).toFixed(2)}</td><td>${(Number(item.quantity || 0) * Number(item.price || 0)).toFixed(2)}</td></tr>`).join('');
