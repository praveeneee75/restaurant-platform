...
function thermalPrintHtml(job) {
  ...
  const templateCss = template === 'BORDERLESS' ? 'th,td{border:0;border-bottom:1px solid #aaa}.summary{border:0}' : template === 'COMPACT' ? 'body{font-size:9px}th,td{padding:2px 1px;border:0;border-bottom:1px dashed #999}' : template === 'MODERN' ? '.title{border:0;border-left:4px solid #000}th{background:#000;color:#fff}.summary{border:0;border-top:2px solid #000}' : template === 'MINIMAL' ? '.title,.meta,.thanks{border:0}th,td,.summary{border:0}th{border-bottom:1px solid #000}.total{border-top:2px solid #000}' : '';
...
async function createWindow() {
  ...
  try {
    ...
    if (health?.database?.status === 'ERROR') {
      throw new Error(`This POS installation cannot open its local database runtime. Reinstall the latest official build. Details: ${health.database.message || 'database runtime unavailable'}`);
    }
