...
document.addEventListener('click', async (event) => { const button=event.target.closest('[data-approve-qr]'); if(!button)return; button.disabled=true; try { const response=await fetch('/qr/orders/approve',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({restaurantId,actor:{id:sessionUser.id,role:sessionUser.role},orderId:Number(button.dataset.approveQr)})}); const data=await response.json(); if(!response.ok||data.success===false)throw Error(data.message||'QR approval failed'); billingStatus.textContent=data.message; window.dispatchEvent(new Event('pos:notifications-changed')); await load(); } catch(error){billingStatus.textContent=error.message;button.disabled=false;} });
