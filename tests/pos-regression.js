...
const thermalEscPos = fs.readFileSync(path.join(root, 'pos-app/electron/thermalEscPos.js'), 'utf8');
...
assert.match(live, /removeSelectedItem/);
assert.match(live, /askPosConfirmation/);
assert.match(live, /await askPosConfirmation/);
assert.doesNotMatch(live, /\bconfirm\(/);
assert.doesNotMatch(liveHtml, /Save Note/);
assert.doesNotMatch(live, /data-minus="\$\{item\.key\}"/);
assert.doesNotMatch(live, /data-note=/);
assert.match(live, /notes: item\.notes \|\| ""/);
...
assert.match(waiter, /function taxInclusiveUnitPrice/);
assert.match(waiter, /Total \(tax included\)/);
assert.match(waiter, /cancelSelectedOrder/);
assert.match(server, /forcePin \|\| !privileged/);
...
assert.match(live, /restoreCurrentContextOrder/);
...
assert.match(live, /loadOpenOrdersForCurrentContext/);
assert.match(server, /o\.table_id IS NULL AND o\.order_type = \?/);
assert.match(server, /notes: row\.notes \|\| ''/);
...
assert.match(electronMain, /buildThermalEscPos/);
assert.match(electronMain, /pDataType = "RAW"|rawPrint\.ps1/);
assert.match(thermalEscPos, /KOT -/);
assert.match(thermalEscPos, /Special Note/);
assert.match(thermalEscPos, /GSTIN:/);
assert.match(thermalEscPos, /FSSAI:/);
assert.match(thermalEscPos, /CGST @/);
assert.match(thermalEscPos, /SGST @/);
assert.match(thermalEscPos, /Total GST/);
assert.match(thermalEscPos, /Reverse chg\./);
assert.match(thermalEscPos, /Authorised Signatory/);
