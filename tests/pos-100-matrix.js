...
async function main() {
  ...
  const validComboIds = new Set(db.prepare('SELECT DISTINCT combo_id FROM combo_items WHERE active = 1').all().map(row => Number(row.combo_id)));
  combos = combos.filter(combo => validComboIds.has(Number(combo.id)));
  ...
  for (let index = 0; index < 100; index += 1) {
    ...
    const orderKitchenIds = afterSaveDb.prepare('SELECT DISTINCT kitchen_id FROM order_items WHERE order_id = ? AND kitchen_id IS NOT NULL').all(saved.orderId).map(row => row.kitchen_id);
    afterSaveDb.close();
    if (afterSavePrintJobs.length !== firstPrintJobs.length) throw new Error(`case ${index + 1}: Save incorrectly created a KOT print job`);
    const kdsBeforeSecondSubmit = await json('GET', `/kds/orders?restaurantId=${restaurantId}&role=OWNER&kitchenIds=${orderKitchenIds.join(',')}`);
    const kdsBeforeItems = (kdsBeforeSecondSubmit.orders || []).find(order => Number(order.orderId) === Number(saved.orderId))?.items || [];
    if (kdsBeforeItems.some(item => Number(item.orderItemId) === Number(draftLine.order_item_id))) throw new Error(`case ${index + 1}: saved draft leaked into KDS before Submit KOT`);
    if (!kdsBeforeItems.some(item => Number(item.orderItemId) === Number(originalLine.order_item_id))) throw new Error(`case ${index + 1}: submitted item missing from KDS`);
    await json('POST', '/orders/submit-kot', { restaurantId, actor, orderId: saved.orderId });
    const kdsAfterSecondSubmit = await json('GET', `/kds/orders?restaurantId=${restaurantId}&role=OWNER&kitchenIds=${orderKitchenIds.join(',')}`);
    const kdsAfterItems = (kdsAfterSecondSubmit.orders || []).find(order => Number(order.orderId) === Number(saved.orderId))?.items || [];
    if (kdsAfterItems.filter(item => Number(item.orderItemId) === Number(draftLine.order_item_id)).length !== 1) throw new Error(`case ${index + 1}: submitted draft did not appear exactly once in KDS`);
    ...
    const invoice = await json('GET', `/orders/invoices/${saved.orderId}?restaurantId=${restaurantId}&role=OWNER`);
